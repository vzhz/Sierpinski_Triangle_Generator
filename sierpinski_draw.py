...
def run(script_version):
    if script_version == "ft":
        print("Hello, triangle-lover!")
        draw_triangle()
        # eventually ask if they want to see others and rerun (but from inside that function?)
    elif script_version == "fp":
        print("Hello, visualization-watcher!")
        draw_pyramids()
        # ask if they want to see other versions
    elif script_version == "s":
        print("Hello, triangle-learner!")
        draw_pyramids_slow()
        # ask if they want to see other versions (since they may have gotten confused and done the slow version and want to see fast now)
    else:
        print("Please run script again and choose a valid option.")
        exit()
...
def draw_triangle():
    ...
    varibles = [random_point() for i in range(3)]
    i = random.randint(0,2)

    #def draw_either(chosen_one, P, draw_pixel)
    count = 0
    while count <= 100000:
        i = (i + random.randint(0,2))%3
